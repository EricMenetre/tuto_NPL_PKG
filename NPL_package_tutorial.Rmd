...
The NPL package was created to help researchers from the NeuroPsychoLinguistic lab of the University of Geneva to perform behavioral and EEG analyses as well as helping to report the results and get the code easier to read.
...
Before installing or using the NPL package, please make sure that all the following packages are installed on your computer and up to date.
...
Creating such a package necessitated a lot of time and effort. Please, if you used this package in your analyses, cite it using: 
...
The following section is divided in three. First the function designed to help performing behavioral analyses, then the function dedicated to EEG analyses and finaly the genric function usable to help writing in Rmarkdown or reporting statistical results. For each function, detailed example and and explanations will be provided. 

## Behavioral analyses

To illustrate the following function, a dataframe containing reaction time is provided in the tutorial GitHub repo. The file is named data_Stroop_RT.xlsx.
...
data_Stroop_RT$trigger <- as.character(data_Stroop_RT$trigger)
data_Stroop_RT$group <- as.character(data_Stroop_RT$group)
```
...
the reaction times are not equally distributed from each side of the median. Two options are possible here. Either use non-parametric, generalized linear methods (using Gamma distribution), or cut the extreme values on the right side of the distribution. However, since there is a variability across groups or subjects, the cleaning of the extreme reaction times needs to be done per group or per subject. Again, two solutions are possible. Either use Excel, calculate one average and standard deviation per subject and manually change the RT to missing values or use the `clean.sd()` function directly in R.
The `clean.sd()` function is designed to automatize this process. 
...
The function rreturns several outputs. First an histogram and QQ-plots of the distribution of the RT before and after cleaning. A table containing the superior and inferior limits  has also been created. It also returns a message specifying that the work is done and the results have been checked for the first two groups. The same messages specifies that a vector named clean.val has been created in the environement and we need to add this vector as a new variable of our data frame (and to keep things clean, we will remove this vector from the environement):
...
Here the function was applied on the group level. It also can be possible to apply it on the subject level, by specifying the subject variable as df.var.group.
...
ggplot(aes(x = subject, y = RT))+
  geom_boxplot()+
  theme_minimal()+
  labs(title = "Boyplot showing the differences in RT among subjects - young adults group")+
...
- group.var: Groupping variable for example age group. Use the raw name of the column e.g. group instead of data$group without "" either. The default is NULL, use it like this without groupping variable in your data. This argument is linked to the groupping one. if groupping = TRUE, specify the group.var argument.
...
- one boyplot per color included in the Stroop task to assess variability between colors.



### LMM_check

After performing a mixed model analysis, it is very important to check of the residus are normally distributed. To do so, either retrieve each plot by its own command line or use the `LMM_check()` function. As the previous one, this function is informative only for linear models.
...
We will assume that all the procedure has been followed and led us to the following model: 
```{r, message=FALSE, warning=FALSE}
library(lme4)
library(lmerTest)
...
### ICC_ranef
An often forgotten step in the analysis of moxed models is the interpretation of the intra-class correlation. This measure translate the cohesion of the data in one variable used as random effect in a mixed model. If the ICC is large, this means that there are large differences among the different level of the variable. A low ICC shows a good cohesion between the level of the variable.
...
The function returns a data frame containing the names of the random variables with their respective ICC. Here the subject variable explains 34.68% of the total variance of the DV. item.color shows a much better cohesion with an ICC of 0.68%.



### mod_fitting

The hierarchical approach is often recognized as the gold standard regarding the selection of the mixed model to use. However, this approach is not very convenient, necessitating for the user to compare sometimes a dozen of models among them. To facilitate this process, the user could use the `mod_fitting()` function.
...
The function returns a dataframe arranging the models by the smallest AIC. Here the... 
The dataframe includes as well the information of the loglikelihood, df_resid. If the models is REML, the cAIC is estimated with its DF. To confirm, the model type and the REML information are mentioned as well as the formula. 
...
Another option to compare models between them is to perform a chi-squared test between models. This can be achieved using the `anova()` function. However, this function can compare only models two by two. The `cross_anova_models()` function helps interpreting the results by designing a matrix confronting the anova p-valued model by model, facilitating the interpretation. To use this function on the data here, we will use the list of models used in the `mod_fitting()` function.
...
Here the m4 should be the one to retain.


### std_error

Oddly, in the base package, no function is designed to calculate the standard error of an array. The `std_error()` function calculates it as: 
...
Do not use this function inside an Rmarkdown document, always from a script.
...
Let's use the function on the Stroop data (dataframe presented for the Behavioral analyses). But first, we need to create the RT_TVA column containing the RT reduced by 100ms to avoid including the beginning of the articulation on the EEG.
...
In less than a minute, 151 .tva files were created in the tva.files folder. Also be careful to the format of your variables, which should all be numerical.

### delete.spaces.STEN
To import the data into the STEN software, the files should be under the .ep format, rereferenced to average and the names should not contain any space.To rename the files, one can either use a .bat script from Windows, or use the `delete.spaces.STEN()` function.
...
```{r}
list.files("delete.spaces.STEN/Original files names")
...
The dataframe contains all the electrodes in columns. The last two columns contain the time frame information and each file is names "subj_1" to "subj_n".

Now let's imagine we want to plot the data for an electrode :
```{r}
...
- file1: path to the first file to subtract
- file2: path to the second file to subtract
- path_save: path and file name where to save the result of the subtraction.

Be careful, the function always performs file2 - file1. Moreover, the coding is not very efficient, which can result in a long computation time for large files.
...
The function created in the working directory as many files as the number of comparisons present in the tanova .txt file. Here there was two main effects and their interaction, hence the function wrote three .ep files containing the results. It is also noteworthy to mention that the results are dichotomized to be more easily interpreted in Cartool. The default value is 0.05 but it can be set to 0.01 or 0.001 if necessary. 

### find_max_amp_diff
When performing peak analysis, it is sometimes difficult to find the best electrode on which to perform the analysis. The `finc_max_amp_diff()` function helps the user by showing the user the electrodes on which the difference between two conditions is maximal.
...
The function returns a dataframe containing all the information contained in all the .csv files from the Cartool output, combining the different groups.

### read_seg_file
For analysis or visualisation purposes, one might want to import the data contained in the .seg file (containing the fitting information of each map on the GFP). These data can be imported in R using the `read_seg_file()` function.
...
- size_hist: to represent the different maps (colors) of the plot, an histogram is fitted under the GFP. Depending on the size of the output, some white spaces might be apparent if this size is too small. This parameter allows the user to adjust for that. The default is 2, which is a good compromise.
- palette: colors used in the plot uses a palette system (group of colors systematically used for each map). When plotting stimulus aligned and response aligned data, one might prefer using another color set than the one implemented. Across all plots, maps number will have the same colors. by default the color palette is "default", another possibility is to set it on "alternative". This option will use different colors. The color can also be customized by indicating the path to an Excel file containing new color names. The first column should contain the maps numbers and the second column the color names.
...
```{code}
ggsave("my_nice_plot.jpg", dpi = 800) # The dpi argument is the resolution, the minimum for publication is 600
```
